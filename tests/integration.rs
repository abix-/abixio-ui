...
impl AbixioServer {
    fn builder() -> AbixioServerBuilder {
        AbixioServerBuilder {
            volume_count: 4,
            no_auth: true,
    ...
    /// Create an S3 client for this server.
    /// Uses dummy credentials because aws-sdk-s3's anonymous mode
    /// (`allow_no_auth`) fails with "dispatch failure" in this SDK version.
    /// The server runs with `--no-auth` so any credentials are accepted.
    fn s3_client(&self) -> Arc<S3Client> {
        Arc::new(
            S3Client::new(&self.endpoint(), Some(("test", "test")), "us-east-1")
                .expect("create S3 client"),
        )
    }

    fn admin_client(&self) -> Arc<AdminClient> {
...
impl AbixioServerBuilder {
    fn volume_count(mut self, n: usize) -> Self {
        self.volume_count = n;
        self
    }

    #[allow(dead_code)]
    fn no_auth(mut self, v: bool) -> Self {
        self.no_auth = v;
        self
    }

    #[allow(dead_code)]
    fn scan_interval(mut self, s: &str) -> Self {
        self.scan_interval = s.to_string();
        self
    }

    #[allow(dead_code)]
    fn heal_interval(mut self, s: &str) -> Self {
        self.heal_interval = s.to_string();
        self
    }

    #[allow(dead_code)]
    fn mrf_workers(mut self, n: usize) -> Self {
        self.mrf_workers = n;
        self
    }

    fn start(self) -> AbixioServer {
...
// ---------------------------------------------------------------------------
// Helper to run the e2e suite and assert results
// ---------------------------------------------------------------------------
...
async fn run_and_assert(server: &AbixioServer) {
    ...
    eprintln!("  {} tests passed on port {}", total, server.port);
}

// ---------------------------------------------------------------------------
// Tests -- each launches its own server with a different volume layout
// ---------------------------------------------------------------------------
